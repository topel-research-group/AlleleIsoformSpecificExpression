...
group1 = "../../Groups/" + cond1 + "/" + cond1 + "_filtered.vcf.gz"
group2 = "../../Groups/"  + cond2 + "/" + cond2 + "_filtered.vcf.gz"

# Define outfile for the whole analysis
finalout = cond1 + "_" + cond2 + "_results.tsv"
...
with open(finalout, "w") as outfile:
	for chrom in variantpos:
		for pos in variantpos[chrom]:
			...
			alldepths = [int(depth1), int(depth2), int(depth3), int(depth4), int(depth5)]
			allgenos = [geno1, geno2, geno3, geno4, geno5]
			...
			elif (geno1 == geno2) and (geno3 == geno4 == geno5) and (geno1 != geno3):
				thislocus = []
